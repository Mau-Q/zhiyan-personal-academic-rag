...
if (-not (Test-Path -LiteralPath 'Env:DATABASE_URL')) {
    throw 'DATABASE_URL must already be set in the current shell.'
}
...
try {
    ...
    if ($report.online_reranker.sample_count -lt 30) {
    ...
    if (
        ...
        $report.online_reranker.candidate_set_expanded -ne $false
    ) {
