...
| SR-02 | PostgreSQL 作为 `owner_id`、主键适配和生命周期唯一事实源 | `PARTIAL` | `backend/storage/` 已实现 PostgreSQL Schema、owner-scoped 映射、幂等版本/任务、生命周期 CAS；本地 PDF 准备、双索引结果原子提交、ES 隐藏版本写入和先 `INACTIVE` 后清理顺序已接入 | 远程 PostgreSQL 尚未应用迁移或实跑；运行存储、Milvus 版本写入、持久化清理队列和在线权限切换尚未完成 |
| SR-03 | PDF/OCR、章节页码、三种 Chunk Baseline、版本和幂等入库 | `PARTIAL` | 三种切片策略已完成同源受控对比；本地持久化准备、双索引协调及 ES 版本写入已覆盖 PDF SHA、owner 映射、原子版本/任务绑定、页码、Chunk 进度、重放和单侧失败补偿 | 远程 PostgreSQL/ES 实跑、PDF/Chunk 运行存储、OCR、MinIO、Milvus 版本写入及持久化物理删除未完成 |
...
| 阶段 1：数据与索引最小闭环 | `IN_PROGRESS` | 本地 PostgreSQL Schema、幂等 PDF 准备、双索引状态协调、ES 隐藏版本写入及先失效后清理顺序已实现；远程迁移/实跑、Milvus 版本写入、清理队列和在线事实源权限门禁未完成 |
...
2. **索引闭环：** 本地可重放准备、状态协调、ES 隐藏版本写入和失效顺序已完成；下一步实现 Milvus 版本写入器、持久化清理队列和在线 READY 门禁；
