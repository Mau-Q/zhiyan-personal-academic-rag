...
1. **差距冻结：** 对照第 10.3 节区分现有 Hybrid/Reranker 决策/引用门禁与 Fake LLM 边界；
2. **最小真实生成：** 固定一个本机生成模型、Prompt 和解码配置，复用现有 READY Evidence，不同时修改检索；
3. **阶段 2 验收：** 比较 Hybrid 与最佳单路并验证引用、ACL、版本、定位和普通问答稳定回放；MinIO/OCR/性能分别留在各自需求 Gate。
