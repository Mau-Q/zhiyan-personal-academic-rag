...
| SR-08 | 证据上下文、真实 LLM、强制引用、校验与拒答 | `PARTIAL` | Evidence/Citation、`NO_EVIDENCE`；本机固定 Ollama 模型、Prompt/解码和引用编号失败关闭 Canary；在线 READY Evidence 注入专项测试 | 远程 READY 真实生成、Claim 语义支持校验和冲突处理未完成 |
...
| 阶段 2：基础 RAG MVP | `PARTIAL` | 非流式 API、Evidence、拒答、远程 RRF Canary和本机真实生成可运行；远程 READY 真实生成、真实 Reranker 消融与生产验收仍未完成 |
...
2. **下一远程 Gate：** 由用户复用既有 PostgreSQL READY + ES/Milvus RRF Evidence 运行同一真实生成配置；
3. **阶段 2 验收：** 保留 Hybrid 与最佳单路的既有比较，验证引用、ACL、版本、定位和普通问答稳定回放；Reranker、MinIO/OCR/性能分别保持独立变量。
