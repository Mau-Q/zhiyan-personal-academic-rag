...
| 阶段 2：基础 RAG MVP | `PARTIAL` | 非流式 API、Evidence、拒答、远程 RRF Canary、llama3.2/Qwen READY 真实生成闭环和独立模型选型均通过，Qwen 已晋级；3 文档 9 题 v2 验收包远程 `9/9` 通过且保留先前 HTTP 失败谱系 | 最高方案第 10.3 节要求的固定 Reranker 增益验证与保留/回退决策、生产验收仍未完成 |
...
6. **当前独立 Gate：** 保留 Hybrid 与最佳单路的既有比较，在冻结候选与评测集上验证一个固定 Reranker，并按 nDCG、Precision@5、关键失败类别和 P95 作保留/回退决策；MinIO/OCR/性能继续分别保持独立变量。
