...
| SR-06 | 基础规范化和最小路由；改写、多查询和多跳按失败后置 | `PARTIAL` | API 问题输入合同与默认检索链路；`machine/phase3_entry_freeze.json` 冻结 4 个双文档比较失衡 `dev` 样本；`_07` 证明查询拆分无增益并保持关闭；第二变量 `BILATERAL_COMPARISON_ROUTE_COVERAGE_TOP3_V1` 已复用现有 READY 路由和 RRF 完成默认关闭本地实现 | 第二变量尚未准备或运行真实在线配对 dev，不能声称质量增益或进入 `test`；不得扩大为通用改写、多查询或多跳 |
...
| 阶段 3：针对失败类型增强 | `IN_PROGRESS` | 首个变量已可信在线失败并保持关闭；第二个单一变量已完成复用评估、合同和默认关闭本地实现，只在原 RRF 候选上提供两条授权路由 Top-3 覆盖 | 第二变量尚未经过在线 dev；`test/acceptance` 继续封存，300 ms 性能 Gate 独立。下一 Gate 统一准备远程 Control/Treatment、裁决和清理，不增加其他主要变量 |
