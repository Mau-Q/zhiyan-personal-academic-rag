...
| SR-08 | 证据上下文、真实 LLM、强制引用、校验与拒答 | `PARTIAL` | Evidence/Citation、`NO_EVIDENCE`；llama3.2 远程 READY + ES/Milvus RRF 真实生成闭环通过，报告 SHA-256 `E2231FADABB368209F976B2BAB99F4E1D841ACB3053C45A07B1ADDC7B386E937`；固定公开 Evidence 模型选型以 Qwen `4/4` 对 llama3.2 `2/4` 晋级，报告 SHA-256 `21C27EAE18848962FC25A879AC620F989F4DD9690C6EB67A10236BE71DAF788B` | Qwen 远程 READY 闭环、固定普通科研问答验收包、Claim 语义支持校验和冲突处理未完成 |
...
| 阶段 2：基础 RAG MVP | `PARTIAL` | 非流式 API、Evidence、拒答、远程 RRF Canary、llama3.2 READY 真实生成闭环和独立模型选型均通过，Qwen 已晋级；Qwen READY 端到端闭环、固定普通科研问答验收包、真实 Reranker 消融与生产验收仍未完成 |
...
4. **下一独立 Gate：** 不修改检索参数，复用 PostgreSQL READY + ES/Milvus RRF Evidence，对冻结摘要的 Qwen 执行真实生成、引用、ACL、版本、删除后 403 和三路清理闭环；
5. **阶段 2 验收：** 保留 Hybrid 与最佳单路的既有比较，验证引用、ACL、版本、定位和普通问答门禁稳定回放；Reranker、MinIO/OCR/性能分别保持独立变量。
