...
| SR-06 | 基础规范化和最小路由；改写、多查询和多跳按失败后置 | `PARTIAL` | API 问题输入合同与默认检索链路；`machine/phase3_entry_freeze.json` 已从人工确认 `dev` 失败冻结 4 个双文档比较失衡样本和唯一实验变量 `BILATERAL_COMPARISON_QUERY_DECOMPOSITION_V1` | 变量尚未实现，方案阶段 3 仍未启动；不得扩大为通用改写、多查询或多跳 |
...
| 阶段 3：针对失败类型增强 | `NOT_STARTED` | 入口 Gate 已冻结 4 个 `dev` 双文档比较失衡样本、基线/增益/不退化线和单一变量 `BILATERAL_COMPARISON_QUERY_DECOMPOSITION_V1`；未实现增强，故阶段仍未启动 | `test/acceptance` 保持封存；300 ms 性能硬化继续为独立 Gate，不与失败类型能力同时改变 |
...
阶段 3 当前只完成入口冻结：`docs/PHASE_3_ENTRY_FREEZE.md` 与
`machine/phase3_entry_freeze.json` 固定 4 个 `dev`
`CROSS_DOCUMENT_IMBALANCE` 样本、原路径配对 control、单一双侧查询拆分变量、
目标增益、关键类不退化线、成本预算、默认关闭回滚和
`dev/test/acceptance` 隔离。未实现查询拆分，未读取 `test/acceptance`，
未改变默认 RRF 或启用 Reranker，方案阶段 3 仍为 `NOT_STARTED`。
