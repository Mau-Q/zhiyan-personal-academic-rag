...
| SR-11 | 200～500 条初始评测，正式 800～1500 条与专家复核 | `PARTIAL` | 15 题 Canary；500 条 V1 合同、双标注谱系、拆分/泄漏校验和排名指标框架 | 真实 500 条采集、双人标注、专家仲裁和盲测锁定未完成 |
...
| 方案阶段 | 当前判断 | 说明 |
|---|---|---|
| 阶段 0：范围冻结与基线 | `IN_PROGRESS` | 有本地范围、15 题 Canary 和正式评测框架，但真实 500 条、目标语料/并发、真实 ES/Milvus 基线与模型资源预算未冻结 |
...
1. **范围与评测线（共享）：** 冻结目标语料量、峰值并发、硬件预算和 SLO；框架目标已固定为 500 条，下一步完成真实采集、双标注、仲裁与盲测锁定；
2. **本地工程线（A）：** 已完成同一 Canary 的 BGE-M3/RRF 和正式排名指标工具；等待锁定 `dev/test` 后运行四路正式对比；
