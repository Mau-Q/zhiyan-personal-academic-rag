...
| SR-06 | 基础规范化和最小路由；改写、多查询和多跳按失败后置 | `PARTIAL` | API 问题输入合同与默认检索链路；`machine/phase3_entry_freeze.json` 冻结 4 个双文档比较失衡 `dev` 样本；唯一变量的默认关闭实现、Control 原问题 `4/4`、Treatment 规划 `4/4`、隔离 Windows 用户运行入口及报告独立裁决器已完成本地契约/静态检查 | 尚未运行真实 PostgreSQL READY + ES/Milvus + RRF 配对 dev 回放与不退化判定；不得把运行器或裁决器就绪写成质量通过，也不得扩大为通用改写、多查询或多跳 |
...
| 阶段 3：针对失败类型增强 | `IN_PROGRESS` | 入口 Gate 已冻结 4 个 `dev` 双文档比较失衡样本、基线/增益/不退化线和单一变量；默认关闭实现、4/4 Control/Treatment 纯规划检查、隔离 Windows 用户运行入口及报告裁决准备已完成 | 真实在线配对 dev 质量与不退化尚未运行；`test/acceptance` 保持封存；300 ms 性能硬化继续为独立 Gate |
...
当前没有远程报告，PASS 分支也只允许形成默认关闭的 dev 候选草案。
