...
| 阶段 2：基础 RAG MVP | `PARTIAL` | 非流式 API、Evidence、拒答、远程 RRF Canary、llama3.2/Qwen READY 真实生成闭环和独立模型选型均通过，Qwen 已晋级；3 文档 9 题普通科研问答验收包已本地冻结 | 验收包远程逐文档执行、真实 Reranker 消融与生产验收仍未完成 |
...
5. **当前独立 Gate：** 3 篇指定论文、9 个普通科研问题的私有验收包已本地冻结；用户需在不修改检索参数的前提下逐文档执行并返回脱敏报告；
