...
| SR-11 | 200～500 条初始评测，正式 800～1500 条与专家复核 | `PARTIAL` | 15 题 Canary；原方案兼容合同；用户批准的风险驱动 80 题工程策略 | 80 题仅用于工程决策；原方案规模、双人工标注和正式盲测仍未完成 |
...
| 方案阶段 | 当前判断 | 说明 |
|---|---|---|
| 阶段 0：范围冻结与基线 | `IN_PROGRESS` | 有本地范围、15 题 Canary、原方案兼容框架和风险驱动策略，但 80 题工程集与原方案正式规模均未完成，目标语料/并发和远程资源也未冻结 |
...
1. **范围与评测线（共享）：** 冻结目标语料量、峰值并发、硬件预算和 SLO；近期按用户决策先建默认 80 条工程集，GPT 初标，人工确认 Acceptance、冲突和高风险题；
2. **本地工程线（A）：** 已完成 BGE-M3/RRF 和排名指标工具；80 题就绪后运行四路对比，只在出现稳定排序缺口时接入重排；
3. **远程基础设施线（B）：** 先完成主机盘点和同一 `main` 基线验证，再按 PostgreSQL、Elasticsearch、Milvus、模型服务逐项建立真实状态和性能基线。

80 题工程基线通过不自动满足 SR-11，也不代表方案阶段 0 完成。只有明确恢复原方案验收口径，并让范围、正式评测、真实基础设施基线和资源选型共同形成可审计证据后，才能按最高方案关闭阶段 0。
