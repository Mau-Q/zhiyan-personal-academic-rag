...
| SR-06 | 基础规范化和最小路由；改写、多查询和多跳按失败后置 | `PARTIAL` | API 问题输入合同与默认检索链路；`machine/phase3_entry_freeze.json` 冻结 4 个双文档比较失衡 `dev` 样本；`_07` 证明查询拆分在该 4 条上无增益，路由覆盖 `_01` 证明选择改变仍无 Recall@3/nDCG@3 或双侧命中增益，两个变量均保持关闭 | 结果只适用于冻结 4 条 `dev`，不能扩大为方法类别无效；不得进入 `test`、调参或重跑。未来重开阶段 3 时先定位 Evidence 在 ES/Milvus/RRF Top-50、最终 Top-3、Chunk/邻块/Evidence Set、标签与指标中的丢失层，再选择新变量 |
...
| 阶段 3：针对失败类型增强 | `IN_PROGRESS` | 首个查询拆分变量和第二个路由覆盖变量均已完成可信在线 dev 比较并保持关闭；第二变量虽在 4/4 目标上生效、3/4 改变选择，但 Control/Treatment 仍同为双侧 `0/4`，Recall@3 与 nDCG@3 增益均为 0。该结论只覆盖冻结 4 条 `dev`，不否定整个方法类别 | `_01` 清理 9/9、READY 失败关闭和删除后 403 均通过，无需恢复且不得复用；`test/acceptance` 继续封存，300 ms 性能 Gate 独立。未来如重开本阶段，先运行比较失败定位 Gate，再按丢失层选择新变量；当前不激活且不阻塞 Phase 4 |
| 阶段 4：Claim–Evidence 可靠性 | `PARTIAL` | 已复用结构化 Claim 与授权 Evidence 建立确定性单/多 EvidenceSet 核心，覆盖 owner、活动版本、Chunk 身份、数字/单位/比较/限定、部分支持、冲突及最多一个同版本邻块；默认 audit-only | 该本地核心不等于通用语义蕴含或人工真值，Precision、负例拒绝率和人工一致率仍不可测；当前 NLI 已拒绝，不启用在线 NLI/LLM 硬裁决 |
