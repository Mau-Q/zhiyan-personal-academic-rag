...
| 阶段 2：基础 RAG MVP | `PARTIAL` | 非流式 API、Evidence、拒答、远程 RRF Canary、llama3.2/Qwen READY 真实生成闭环、模型选型和 3 文档 9 题 v2 均通过；固定 BGE Reranker 本地质量门及 Windows RTX 4090 组件 P95 通过，决定保留进入受控在线集成 | 固定 Reranker 默认在线路由、失败回退与组合检索 P95 尚未验收；生产验收仍未完成 |
...
8. **当前独立 Gate：** 在 PostgreSQL READY/ACL 与 ES/Milvus RRF 之后接入固定 Reranker，保持候选集合不扩张并验证失败回退及组合检索 P95；MinIO/OCR/通用性能继续保持独立变量。
