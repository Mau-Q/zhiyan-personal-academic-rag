...
Run 05 的 `combined P95=317.61022 ms` 仍高于目标 `300 ms`；因此不能把本地
变更或 Run 05 写成 300 ms 已达标。
...
这些是代码与合同证据，不是远程性能证据。要判断是否缩小 Run 05 的
`317.61022 ms` 与 `300 ms` 的差距，仍需在原目标硬件、冻结模型/输入/候选边界
...
本地随后新增 V2 Batch-20 作为单变量性能实验，V1 保持冻结；V2 尚无远程结果，
不能预先声称其会降低 P95 或通过门禁。

## 5. 明确未处理的事项

- 正式 Acceptance、真实用户评价和生产运维仍需要相应外部参与或独立工作流；
- 本轮没有更换模型、放宽阈值、减少候选、引入重复问题缓存、修改默认 RRF 或修改冻结 V1 配置；
- 本轮没有重开 Phase 3 排序优化、查询拆分、路由覆盖、NLI 或阶段 5；
- V2 Batch-20 仍需用户在 Windows 目标硬件上独立执行；本轮没有运行真实生成或
  正式 Acceptance，Mac 只根据用户提供的脱敏摘要判断远程性能。
