...
Ollama provider 现对同一 provider 实例使用惰性、线程安全的 HTTP/1.1 持久连接：
完整读取响应后复用连接；服务端声明关闭或发生传输异常时丢弃连接；进程/应用关闭时
可显式调用 `close()`。该变更只减少连接建立开销，不缓存查询文本、向量或模型身份，
不改变 `/api/embeddings`、`/api/embed` 的端点选择、请求内容、模型、截断和失败关闭
语义。它是新的单变量性能实验，尚无 Windows RTX 4090 远程结果，不能预先声称会
降低 P95。
...
- Ollama 持久连接的复用、服务端关闭后的连接替换和显式关闭；
...
- Ollama 单查询轻量端点、显式模型驻留、READY 后 Chunk snapshot 预热与有界 Milvus
  在线逻辑行读取均已获得有限远程观测；Run 13/14 的 combined P95 分别为
  `302.983085/309.49054 ms`，未通过稳定 300 ms 性能门禁。新增的 Ollama 持久连接
  复用目前只有本地合同测试，仍需新的远程 Run 验证；本轮没有运行真实生成或正式
  Acceptance，Mac 只根据用户提供的脱敏摘要判断远程性能。
