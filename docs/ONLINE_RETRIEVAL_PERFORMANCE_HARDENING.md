...
Run 13/14 已完成远程观测，但尚未证明稳定通过 300 ms。

### 2.12 Ollama 有界连接复用与并发保持

针对 Run 13/14 中 Query Embedding P95 `162.41544/168.735645 ms` 的剩余成本，
Ollama provider 使用同一 provider 实例内最多两条惰性、线程安全的 HTTP/1.1 持久连接：
完整读取响应后复用连接；服务端声明关闭或发生传输异常时丢弃连接；进程/应用关闭时
可显式调用 `close()`。两条连接保留 query prewarm 与 READY 路由模型身份校验的并行
能力，同时限制无界连接增长。该变更只管理连接，不缓存查询文本、向量或模型身份，
不改变 `/api/embeddings`、`/api/embed` 的端点选择、请求内容、模型、截断和失败关闭
语义。

Run 15 在前一版单共享连接的 `a2d4f17` 上完成：Query Embedding P95 降至
`133.034825 ms`，但 READY 物理验证墙钟 P95 升至 `218.600071 ms`，combined P95
升至 `364.802135 ms`。这暴露出单连接锁可能阻塞 prewarm 与路由身份请求的并行路径，
但一次远程运行不能把全部回归归因于该机制；因此当前改为有界两连接池，并新增并发
合同测试，仍需新的 Windows RTX 4090 Run 验证，不能预先声称会降低 P95。
...
- Ollama 最多两条持久连接的复用、并发请求、服务端关闭后的连接替换和显式关闭；
...
安全边界内的有界 Milvus 读取不足以单独收敛性能债。保留该语义不变的硬化；这里的“收口”
仅指完成该变量的证据归档、拒绝虚假宣称和避免同变量盲目重跑，不代表性能优化停止。
鉴于质量优先且不赶进度，下一步重新打开性能工作流：先对物理校验请求、Embedding 与
关键路径做更细 profiling，再一次只选择一个结构性变量验证。Demo/界面/README 可以并行
准备，但所有展示必须明确标注 `combined P95 > 300 ms`，且不得改变 V1、RRF、候选、ACL
和失败关闭合同。

随后用户在提交 `a2d4f17a010afc2476f97aaae33dd7514313ffa0`、同一 V1 配置下完成 Run 15：
30/30 `APPLIED`，但 `status=FAIL`。`base P50/P95=216.73785/232.748775 ms`，
`combined P50/P95=345.43015/364.802135 ms`，Reranker P95 为 `131.01399 ms`，
Query Embedding P95 为 `133.034825 ms`。READY route resolution P95 为 `220.257805 ms`，
READY PostgreSQL lookup P95 为 `0.77762 ms`，物理验证墙钟 P95 为 `218.600071 ms`，
ES 物理校验工作 P95 为 `28.36872 ms`，Milvus 物理校验工作 P95 为 `217.55837 ms`，
Chunk snapshot P95 为 `219.27806 ms`。ES validation/query/total work P95 为
`2.68247/8.100165/10.85621 ms`，Milvus validation/query-embedding/ANN/total work P95
为 `1.56294/133.034825/6.575916/8.00003 ms`，后端并行墙钟 P95 为 `11.84879 ms`，
READY revalidation P95 为 `0.883605 ms`，RRF P95 为 `0.10006 ms`，retriever total P95
为 `232.562619 ms`。清理 3/3、删除后 403、无 fallback/候选扩张/候选越界和分段状态均
通过；报告 SHA-256 为 `06EAE42C3027D6DD1F3B3C1B667530EFF3BD1FE5912B0441667479274BEFC366`。

Run 15 相比 Run 14 的 Query Embedding P95 下降 `35.70082 ms`，但 READY 物理验证墙钟
P95 上升 `106.710731 ms`，combined P95 上升 `55.311595 ms`，因此不能把单共享连接
复用晋级默认路径，也不能把一次远程回归完全归因于它。Run 15 的分段结果提示：单共享
连接可能让 prewarm 与 READY 路由模型身份请求互相等待；当前本地已改为最多两条连接的
有界复用并补充并发合同测试。

Run 16 在提交 `29c87420c0b3d720c68f4e065874b87c9934a767`、同一 V1 配置下完成：
`status=PASS`、30/30 `APPLIED`，`base P50/P95=138.29465/156.808445 ms`，
`combined P50/P95=269.22035/287.73689 ms`，Reranker P95 为 `132.908545 ms`，
Query Embedding P95 为 `147.96689 ms`。READY route resolution P95 为 `95.08563 ms`，
READY PostgreSQL lookup P95 为 `0.806855 ms`，物理验证墙钟 P95 为 `93.247355 ms`，
ES/Milvus 物理校验工作 P95 为 `91.25044/91.25921 ms`，Chunk snapshot P95 为
`94.023235 ms`。Milvus validation/query-embedding/ANN/total work P95 为
`1.4526/147.96689/5.76911/7.297981 ms`，后端并行墙钟 P95 为 `60.590285 ms`，
READY revalidation P95 为 `0.914765 ms`，RRF P95 为 `0.112385 ms`，retriever total
P95 为 `156.60407 ms`。清理 3/3、删除后 403、无 fallback/候选扩张/候选越界和分段
状态均通过；`stable_error_code=NONE`，报告 SHA-256 为
`1A325C4EBFBE045DBCAF8CBB372C652FFC899F225B97ED8DEE863E762CE0E219`。

Run 16 是两连接池修正后的首次通过样本：相对 Run 15，READY 物理验证墙钟 P95 下降
`125.352716 ms`，combined P95 下降 `77.065245 ms`，支持该修正恢复并行性的判断。

Run 17 在同一提交、同一 V1 配置下完成确认：`status=PASS`、30/30 `APPLIED`，
`base P50/P95=135.29595/162.213044 ms`，`combined P50/P95=265.31965/292.114209 ms`，
Reranker P95 为 `132.391025 ms`，Query Embedding P95 为 `153.263471 ms`。READY route
resolution P95 为 `94.36018 ms`，READY PostgreSQL lookup P95 为 `0.76885 ms`，物理验证
墙钟 P95 为 `92.9869 ms`，ES/Milvus 物理校验工作 P95 为 `92.583755/91.450085 ms`，
Chunk snapshot P95 为 `93.640155 ms`。Milvus validation/query-embedding/ANN/total work
P95 为 `1.489585/153.263471/6.354855/7.859485 ms`，后端并行墙钟 P95 为 `72.069015 ms`，
READY revalidation P95 为 `1.01328 ms`，RRF P95 为 `0.179715 ms`，retriever total P95 为
`162.00639 ms`。清理 3/3、删除后 403、无 fallback/候选扩张/候选越界和分段状态均通过，
`stable_error_code=NONE`；报告 SHA-256 为
`C960799168E4813A6D02E8ACEEE203ECADE6177DF810EAC27004D2AED162D402`。

Run 16/17 是两次连续、同提交同配置的远程通过样本，combined P95 分别为
`287.73689/292.114209 ms`，因此保留最多两条 Ollama 连接的实现，不再为该变量继续
改码或重跑。该结论仅覆盖当前固定 30 样本 Gate 和既定资源边界，不等同于正式 Acceptance、
生产容量或所有负载下的 SLO 证明；V1 Batch-16、默认 RRF、ACL、READY、向量身份和失败
关闭语义保持不变。
...
- Ollama 单查询轻量端点、显式模型驻留、READY 后 Chunk snapshot 预热与有界 Milvus
  在线逻辑行读取均已获得有限远程观测；Run 13/14/15/16/17 的 combined P95 分别为
  `302.983085/309.49054/364.802135/287.73689/292.114209 ms`。Run 15 的单共享连接
  复用不晋级；最多两条连接的本地并发合同测试及 Run 16/17 远程 Gate 均通过，当前
  保留该实现。本轮没有运行真实生成或正式 Acceptance，Mac 只根据用户提供的脱敏
  摘要判断远程性能。
