...
## MVP 175 远程单路 Baseline

两路均使用同一人工冻结 175 题、同一 316 Chunk、`top_k=3` 和现有远程索引身份。严格整题结果如下：
...
分类结果为：精确查找 `21/30 vs 30/30`、单文档事实 `36/50 vs 47/50`、语义改写 `15/30 vs 20/30`、比较 `7/20 vs 8/20`、证据边界 `6/25 vs 4/25`、安全 `0/20 vs 0/20`。前者均为 ES，后者均为 Milvus。

两路同时通过 78 题，仅 ES 通过 7 题，仅 Milvus 通过 31 题，两路均未通过 59 题。138 道可回答合同内仅 ES 通过 6 题，仅 Milvus 通过 31 题；Milvus 是当前更强的单路召回基线。

`NO_EVIDENCE` 和安全题的失败不都是排序失败：前者暴露无证据校准不足，后者暴露当前 Answer API 只有范围授权拒绝、没有有害/敏感请求策略层，且 `RAG_FORBIDDEN_SCOPE` 不应冒充内容安全错误。这些缺口按真实边界保留。

远程 RRF Canary 已为 `14/15`，没有超过 ES；本次 Top-3 互补又明显倾向 Milvus。因此不运行 175 题 RRF，不调整融合参数，不增加重排，仅保留已有最小 RRF 适配器。
