...
3. **共享：范围与评测线**
   ...
   - 保留 15 题作为快速 Canary，近期另建默认 80 条风险驱动工程集；
   - GPT 负责候选和低风险初标，人工确认 Acceptance、冲突与高风险题；原方案正式规模单独记账。
...
- 80 题通过只允许继续工程推进；在恢复原方案评测口径、完成真实 ES/Milvus 基线和模型资源预算前，不将最高方案阶段 0 标记为完成。
