...
最高方案当前处于“阶段 0：范围冻结与基线建立”进行中。仓库内部 M0 已完成下列可审计最小链路，但它不代表最高方案阶段 0 已完成：
...
3. **共享：范围与评测线**
   - 冻结首期知识源、目标语料量、峰值并发、SLO 和模型资源预算；
   - 保留 15 题作为快速 Canary，另建 500 条 GPT 辅助工程集；
   - GPT 负责候选和低风险初标，人工确认 Acceptance、冲突、专业高难题和低风险抽检；原方案双人工口径单独记账。
...
- GPT 辅助 500 题通过只允许继续工程推进；在恢复原方案人工评测口径、完成真实 ES/Milvus 基线和模型资源预算前，不将最高方案阶段 0 标记为完成。
