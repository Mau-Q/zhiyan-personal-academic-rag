...
最高方案当前仍处于“阶段 0：范围冻结与基线建立”。评测 Baseline 与范围/资源/SLO 已冻结，数据身份适配和上游生命周期语义仍待收口。仓库内部 M0 已完成下列可审计最小链路，但它不替代后续数据与索引闭环：
...
3. **共享：范围与评测线**
   ...
   - 175 题真实人工校验、同集 ES/Milvus 单路 Baseline、三种 Chunk 受控 Baseline 及范围/资源/SLO 冻结均已完成；下一步冻结数据身份与生命周期语义。
...
- `owner_id`、`paper_id ↔ document_id` 和上游生命周期目标语义未冻结前，不将最高方案阶段 0 标记为完成。
