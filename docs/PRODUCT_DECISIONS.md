...
| PD-014 | ACCEPTED | 3 论文 15 题保留为固定工程 Canary，不替代 200～500 条初始评测和 800～1500 条正式验收规模 | 新检索变量先跑 Canary，阶段关闭仍需分层标注、专家复核和独立验收集 |
...
| PD-018 | SUPERSEDED_BY_PD-019 | 保留 500 题，GPT 承担候选生成和低风险初标；低风险 `dev/test` 人工抽检 10%～20%，Acceptance 单人确认，冲突才仲裁，专业高难题才专家复核 | 500 题正式兼容集继续保留，但近期工程流程由 Token 约束决策替代 |
| PD-019 | ACCEPTED | Token 成本是后续流程的首要约束；默认复用现有结果、采用确定性脚本和最小充分门禁，必要时允许降低工程集题量与评审质量 | 第二模型审计替代近期人工风险复核；40 条修正直接采用，18 条拒绝题不重生成，形成 482 题低置信工程集；原 500 题和最高方案人工缺口单独保留，不虚报完成 |
