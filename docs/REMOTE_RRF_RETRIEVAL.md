...
当前只完成本地适配器、Fixture Answer API 和 Harness 验证。远程主机下一步使用现有 Index、Collection、316 Chunk 和原 15 题的 RRF 专用预期告警副本，只执行一条 Harness 命令：

```powershell
& $py -m backend.evaluation.harness --cases "remote-es-canary-input-v1/cases-remote-rrf-v1.jsonl" --chunks "remote-es-canary-input-v1/chunks-v1.json" --scope "remote-es-canary-input-v1/authorized-scope-v1.json" --suite-id "remote-rrf-canary-v1" --retrieval-backend "remote_rrf" --remote-config "remote-retrieval-config-v1.json" --output "remote-es-canary-input-v1/remote-rrf-report.json"
```

运行报告继续留在远程主机。不改题目、目标页、`top_k`、阈值或 HNSW 参数；不同时加入重排、真实 LLM 或 500 题远程全量。
