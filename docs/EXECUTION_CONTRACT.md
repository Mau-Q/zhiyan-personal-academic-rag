...
- 用户说“下一步”“进行”或“继续”时，默认授权完成下一个完整、可审计的本地门禁，而不是只执行一个微动作后等待再次确认；
- 一个完整本地门禁包括：恢复实时仓库状态、实现一个有边界的能力、运行专项测试、按风险运行全仓回归、同步阶段文档与机器状态、审查差异并建立独立本地提交；
- 同一门禁内不重复请求确认。连续的本地能力仍按“一次只改变一个主要边界”逐项实施和提交；
- 默认不推送，不连接、部署或重启远程主机。远程操作清单由代理一次性准备，用户亲自执行并返回脱敏原始输出；
- 遇到远程操作节点、破坏性动作、重大架构分叉、合同破坏或无关工作区改动时停止自动推进并请求用户决策。
