...
- Qwen3.7-Plus 已以 20 路并发完成 373/500 个候选，全部为 `enable_thinking=false`、`finish_reason=stop`，且无 reasoning 内容；
- 剩余 127 个候选因账户返回 `AllocationQuota.FreeTierOnly` 暂停，解除免费额度用完即停或完成充值后可断点续跑；
- 373 个结果中有 3 个重复题面，需在回填正式题集前重生成或去重；候选尚未导入正式合同，因此正式题集仍如实保持 `0/500`。
...
- 下一工作是解除 DashScope 免费额度门禁并断点补齐 127 个候选，随后校验、去重并回填 500 题；重排、原方案人工升级、远程模型与生产索引参数保持证据驱动。
...
当前仓库 Harness 已与最高方案建立需求映射。总体仍处于最高方案阶段 0，仓库 M0、本地 RRF M1 和评测工具只是已验证的子基线。当前最宽本地问答执行边界仍为 `LOCAL_API_RRF_HYBRID_FAKE_LLM`；近期评测执行边界为 `GPT_ASSISTED_500_PROFILE_READY / SOURCE_FROZEN_316_CHUNKS / QWEN_CANDIDATES_373_OF_500 / ITEMS_0_OF_500`，原方案执行边界为 `SOURCE_FORMAL_DOUBLE_HUMAN_PENDING`。它们不证明正式 500 题、原方案双人工口径、PostgreSQL、Elasticsearch、Milvus、远程部署、真实 RAG 生成模型或生产参数已完成。PDF、真实 Chunk、私有问题集、标注记录、索引和运行报告不得提交。
