...
- 未完成能力：阶段 2 固定 Reranker 在 ACL/READY 与 RRF 之后的受控在线集成、失败回退和组合检索 P95，正式 MinIO 适配、OCR、目标规模性能验收、无证据校准和安全策略层；
...
最高方案阶段 0、阶段 1 已完成，阶段 2 仍为 `IN_PROGRESS`。评测执行边界为 `MVP_INITIAL_175_HUMAN_VALIDATED / ES_85_OF_175 / MILVUS_109_OF_175 / THREE_CHUNK_STRATEGIES_BM25_15_OF_15_VECTOR_12_OF_15_NO_WINNER / RRF_175_DEFERRED / ENGINEERING_ITEMS_500_FOUR_BACKENDS_COMPLETE / REMOTE_RRF_CANARY_14_OF_15_NO_GAIN / FIXED_BGE_RERANKER_TEST100_TARGET_COMPONENT_GATE_PASS_CONTROLLED_ONLINE_INTEGRATION_PENDING / REMOTE_READY_LLAMA3_2_GENERATION_PASS / REMOTE_READY_QWEN3_14B_THINK_FALSE_PASS / QWEN3_14B_MODEL_SELECTION_V4_PROMOTED / PHASE2_ACADEMIC_QA_V2_3_OF_3_DOCUMENTS_9_OF_9_CASES_PASS`。固定 Reranker 在不改变候选、Embedding、RRF 或生成的情况下通过本地质量门和 Windows RTX 4090 组件 P95，决定保留进入受控在线集成；本次 `P95=188.22683 ms` 不包含召回与融合，因此线上默认仍保持原路由。远程证据已证明两种生成模型均可消费 PostgreSQL READY + ES/Milvus RRF Evidence，并完成引用、稳定回放、删除后 403 和三路清理；固定四题 v4 进一步确认 Qwen `4/4` 晋级。普通科研问答 v2 的 3 篇文档各 `3/3`、合计 `9/9` 已通过；第 3 篇先前的 Answer HTTP 失败报告作为失败谱系保留，最终 retry2 以报告 SHA-256 `3C106423AB3575B11B3B0142A66F19A2C949B8BAED3457F1BCA101A9931302FA` 通过。`citation_stable_replay=true`，`byte_stable_replay_observed=false` 仅为自然语言观测项；运行时私有数据与报告继续只保留在被忽略的 `runtime/`。

## Next gate

1. **受控在线 Reranker 集成：** 只在 PostgreSQL READY/ACL 与 ES/Milvus RRF 返回既有候选之后调用固定 Cross-Encoder；不得扩张候选、改变召回/RRF/生成参数或让模型先于权限门禁读取内容；
2. **组合门禁与回退：** 验证模型不可用、身份漂移和推理失败时回退既有 RRF，测量包含 READY 路由、ES/Milvus 召回、RRF 和 Reranker 的组合 P95；通过前默认路由保持不变。
