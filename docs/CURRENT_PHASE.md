...
- 未完成能力：Qwen v4 固定四题最终晋级、阶段 2 固定普通科研问答验收包、真实 Reranker 消融、正式 MinIO 适配、OCR、目标规模性能验收、无证据校准和安全策略层；
