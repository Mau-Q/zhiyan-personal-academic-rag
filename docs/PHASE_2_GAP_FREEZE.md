...
- 为满足最高方案第 10.3 节“固定 Reranker 增益验证与保留/回退决策”的退出条件，下一独立 Gate 必须在冻结候选与评测集上验证一个 Cross-Encoder，并记录 nDCG、Precision@5、关键失败类别和 P95；不得与生成或检索参数变化一起引入；
- Claim 语义支持、冲突处理、正式 MinIO、OCR 和目标规模性能继续由各自后续 Gate 跟踪。

阶段 2 因此仍为 `IN_PROGRESS/PARTIAL`。llama3.2 与 Qwen 的远程 READY 真实生成闭环、Qwen 最终晋级和固定普通科研问答验收包远程 `3/3` 文档、`9/9` 问题均已完成；固定 Reranker 增益验证与保留/回退决策仍未完成。后续不把它与 MinIO/OCR、性能或生成变量混合。
